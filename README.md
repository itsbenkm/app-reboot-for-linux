...
**Latest Updates:**
- **Fixed an empty-save wipe at boot:** The periodic safety-net save could fire ~2 minutes into boot — before the restore had opened anything — and overwrite the saved session with an empty scan, so nothing was restored. The saver now refuses to overwrite a non-empty session with an empty scan, and the first post-boot periodic save is delayed (2→5 min) so it can't race the restore.
